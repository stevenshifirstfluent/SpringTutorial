...
public class JpqlTutorialQuiz21Application {
	...
	@Bean
    CommandLineRunner commandLineRunner(CourseService courseService, StudentRepository studentRepo, DepartmentRepository departmentRepo, CourseRepository courseRepo) {
        return args -> {
            // Save departments
            ...
            List<Course> courses = courseService.findCoursesByStudentMatricNo("A00001A");
            courses.forEach(course -> System.out.println(course.getCode() + ": " + course.getDescription()));
        };
    }
}
